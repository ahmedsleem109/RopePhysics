...
CaseResult runHelix(const std::string& outDir) {
    ...
    const HelixShape shape = measureHelix(rod, nSegRef - 1);
    const double radius = shape.radius, pitch = shape.pitch;

    Csv csv(outDir, "helix.csv", "segments,radius,radius_ref,pitch,pitch_ref,turns,steps,converged");
    csv.row(n, radius, Rref, pitch, pitchRef, shape.turns, rep.steps,
            rep.converged ? 1 : 0);
    ...
    res.notes.push_back(fmt("  kappa = %.4g 1/m, tau = %.4g 1/m, %.4f turns over L = 1 m",
                            double(kappa), double(tau), shape.turns));
...
CaseResult runTwistBuckling(const std::string& outDir) {
    ...
    auto growthRate = [&](int n, double phi) {
        ...
        for (int i = 1; i <= maxSteps; ++i) {
            step(rod, p);
            double amp = 0;
            for (const Vec3& x : rod.state.x) amp = std::max(amp, std::sqrt(double(x.x * x.x + x.y * x.y)));
